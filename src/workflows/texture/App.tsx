...
type TextureSizeMode = "original" | "8192" | "4096" | "2048" | "1024" | "512" | "custom";
...
const I18N: Record<UiLanguage, Record<TranslationKey, string>> = {
  en: {
    appSubtitle: "Experimental GLB/ZIP/OBJ texture import for later ColorMix texture baking.",
    language: "Language",
    ...
    bakeColorSourcesTip: "Selects which material channels are baked as printable color. Base Color is the default. Emissive can include HUD/glow graphics; the experimental mode tries to include additional visible material information.",
    textureResolution: "Texture resolution",
    textureResolutionTip: "Maximum texture resolution before sampling. Original keeps the source texture; 4K/2K/1K reduce memory usage but can smooth fine patterns.",
    customTextureEdge: "Max. texture edge",
    sourceResolution: "Source resolution",
    wireframe: "Show wireframe",
    wireframeTip: "Overlays the triangle structure on the original and baked previews.",
    syncViews: "Synchronize views",
    syncViewsTip: "Rotation, panning, and zooming are linked between the original and baked view.",
    view: "View",
    ...
    fitToModelTip: "Fits the current camera to the loaded model bounding box without changing model geometry.",
    resetView: "Reset view",
    resetViewTip: "Resets the current preview camera to the selected view orientation.",
    rebuildPreview: "Rebuild preview",
    rebuildPreviewTip: "Rebuilds the preview scene using the current display, background, axes and wireframe settings.",
    mode: "Mode",
    subdivisionModeTip: "Slicer-safe detail subdivision adds triangles without T-junctions. Off uses only the input model's existing triangles.",
    quality: "Quality",
    qualityTip: "Controls triangle budget, subdivision depth, and error thresholds. Higher levels preserve fine texture patterns better but use much more memory and time.",
    triangleBudget: "Triangle budget",
    triangleBudgetTip: "Upper limit for generated triangles. The budget is not always fully used.",
    maxDepth: "Max. subdivision depth",
    maxDepthTip: "Limits how often a triangle may be recursively split.",
    exportScale: "Export scale",
    exportScaleTip: "Scales OBJ coordinates for slicers. GLB/glTF often uses meters; PrusaSlicer usually interprets OBJ coordinates as millimeters.",
    surfaceDetailSources: "Detail sources for subdivision",
    surfaceDetailSourcesTip: "The default uses Base Color only. Normal/AO/Roughness can be used as additional geometry indicators, but they are not baked as printable colors.",
    reliefGeometry: "Relief geometry",
    reliefGeometryTip: "Experimental: moves vertices using true Height/Bump maps or optionally AO/Roughness proxies. Default is Off.",
    reliefStrength: "Relief strength",
    reliefStrengthTip: "Maximum displacement relative to the model diagonal. Start low, because high values can create artifacts.",
    reliefSmoothing: "Relief smoothing",
    reliefSmoothingTip: "Smooths relief heights before displacement. Stronger smoothing reduces hard artifacts but loses detail.",
    runBake: "Bake texture to triangles",
    runBakeTip: "Runs texture baking with the current colour source, texture resolution, subdivision and bake colour correction settings.",
    baking: "Baking texture…",
    clearBake: "Clear baking",
    clearBakeTip: "Clears the baked result while keeping the loaded source model and current baking settings.",
    exportObj: "Export baked vertex OBJ",
    exportObjTip: "Exports the baked mesh as an OBJ with vertex colours in v x y z r g b format.",
    exporting: "Exporting…",
    standardNote: "Default: Base Color, original texture resolution, slicer-safe detail subdivision, and no relief geometry. The export is a baked OBJ with vertex colors in v x y z r g b format.",
    bakeSummary: "3. Baking summary",
    colorDistribution: "Baked color distribution",
    colorDistributionTip: "Shows similar baked colors as a block map. For photo textures this is more useful than a list of individual hex values.",
    diagnostics: "Diagnostics",
    diagnosticsTip: "Diagnostics are shown compactly. Details can be expanded when needed; structural mesh problems should be repaired in Blender.",
    ...
    noBakedModel: "No baked model. Run texture baking to show the baked preview.",
    ...
    handoffToVertexTip: "Passes the baked vertex-colour OBJ directly to VertexColor 2 ColorMix without saving an intermediate file.",
    handoffProgressTitle: "Send baked OBJ",
    handoffProgressStep: "Send to VertexColor 2 ColorMix",
    handoffPreparing: "Preparing handoff…",
    handoffPrepared: "Handoff prepared: {faces} faces, {vertices} welded vertices. The baked OBJ was sent to VertexColor 2 ColorMix.",
    ...
    bakeColourCorrectionTip: "Adjusts texture/material colours before they are baked into vertex colours. UV coordinates are not changed.",
    ...
    tint: "Tint",
    gamma: "Gamma",
    tipBakeBrightness: "Adds or removes lightness from texture/material colours before baking. Changes are only applied when you click Apply.",
    tipBakeContrast: "Increases or decreases the difference between light and dark texture areas before baking.",
    tipBakeSaturation: "Controls colour intensity before baking. Negative values make colours greyer; positive values make them stronger.",
    tipBakeTemperature: "Shifts texture colours colder or warmer. Negative values are cooler/bluer; positive values are warmer/yellower-red.",
    tipBakeHue: "Rotates hue relative to the source texture colours. 0 means unchanged; -180 and +180 produce the same complementary rotation.",
    tipBakeTint: "Shifts the colour cast between green and magenta before baking. Negative values are greener; positive values are more magenta.",
    tipBakeGamma: "Changes midtone brightness before baking. 1.0 means unchanged; lower values brighten midtones, higher values darken them.",
    apply: "Apply",
    applyBakeColourCorrectionTip: "Applies the pending bake colour correction to the original preview and to the next bake, export and handoff.",
    resetBakeColourCorrectionTip: "Resets bake colour correction to neutral values and clears the current baked result.",
    saveTextureProjectTip: "Saves the Texture Baking project settings and optionally embeds the selected source files in the JSON project file.",
    loadTextureProjectTip: "Loads a Texture Baking project JSON and restores the selected settings and source files.",
    bakeColourCorrectionApplied: "These values are applied during the next texture bake, export and handoff.",
    defaultHints: "Default: Base Color, original texture resolution, slicer-safe detail subdivision, and no relief geometry. Export is a baked OBJ with vertex colours in v x y z r g b format.",
  },
};
...
function ProgressOverlay({ progress }: { progress: ProgressOverlayState | null }) {
  if (!progress) return null;
  const total = Math.max(1, progress.steps.length);
  const completedCount = progress.done ? total : Math.max(0, progress.activeIndex);
  const percent = progress.done ? 100 : Math.round((completedCount / total) * 100);
  return (
      ...
      <div className="progress-card">
        ...
        <ul className="progress-steps">
          {progress.steps.map((step, index) => {
            const state = progress.error && index === progress.activeIndex ? "error" : index < completedCount || progress.done ? "done" : index === progress.activeIndex ? "active" : "pending";
            return (
...
function textureSizeLabel(maxSize: number | null): string {
  return maxSize ? `${textureKLabel(maxSize)} · max. ${formatNumber(maxSize)} px` : "Original";
}
...
function findMtlForObj(objFile: File, files: File[]): File | undefined {
  ...
  return files.find((file) => getFileExtension(file) === "mtl" && directoryNameFromFileName(file.name) === directoryNameFromFileName(objFile.name))
...
async function chooseBestObjFile(files: File[]): Promise<File | undefined> {
  ...
  for (const file of objFiles) {
    ...
    const sameDirectoryAssets = files.filter((candidate) => directoryNameFromFileName(candidate.name) === directory).length;
    score += sameDirectoryAssets * 2;
    try {
      const head = (await file.slice(0, Math.min(file.size, 256_000)).text()).slice(0, 256_000);
      const mtllibMatch = head.match(/^\s*mtllib\s+(.+)$/im);
      if (mtllibMatch) {
        score += 100;
        const referenced = mtllibMatch[1]?.trim();
        if (referenced && files.some((candidate) => getFileExtension(candidate) === "mtl" && normaliseAssetName(candidate.name) === normaliseAssetName(referenced))) score += 80;
      }
...
function assignSceneMetadata(scene: THREE.Object3D, fileName: string, sourceKind: string) {
  scene.name = fileName;
  scene.userData.sourceKind = sourceKind;
  scene.updateMatrixWorld(true);
}

async function parseGltfAssetFiles(files: File[], file: File): Promise<THREE.Object3D> {
  ...
  try {
    ...
    assignSceneMetadata(gltf.scene, file.name, extension === "glb" ? "GLB" : "glTF + external files");
...
async function parseObjAssetFiles(files: File[], preferredObjFile?: File): Promise<THREE.Object3D> {
  ...
  const mtlFile = referencedMtlName
    ? files.find((file) => getFileExtension(file) === "mtl" && directoryNameFromFileName(file.name) === objDirectory && normaliseAssetName(file.name) === normaliseAssetName(referencedMtlName))
      ?? files.find((file) => getFileExtension(file) === "mtl" && normaliseAssetName(file.name) === normaliseAssetName(referencedMtlName))
  ...
  try {
    ...
    assignSceneMetadata(root, objFile.name, mtlFile ? "OBJ + MTL + textures" : "OBJ without MTL");
...
async function resolveInputFiles(fileList: FileList | File[]): Promise<{ files: File[]; archiveCount: number; nestedArchiveCount: number; skippedArchiveEntries: number }> {
  ...
  return { files: resolvedFiles, archiveCount, nestedArchiveCount, skippedArchiveEntries };
}

async function parseModelFiles(fileList: FileList | File[]): Promise<{ scene: THREE.Object3D; mainFile: File; sourceSummary: string; resolvedFileCount: number }> {
  const inputFiles = Array.from(fileList);
  if (inputFiles.length === 0) throw new Error("No file selected.");
  const { files, archiveCount, nestedArchiveCount, skippedArchiveEntries } = await resolveInputFiles(inputFiles);
  if (files.length === 0) throw new Error("No supported model file found. ZIP imports must contain at least one OBJ or GLB/glTF file.");
  const objFile = await chooseBestObjFile(files);
  const glbFile = files.find((file) => ["glb", "gltf"].includes(getFileExtension(file)));

  if (objFile) {
    const scene = await parseObjAssetFiles(files, objFile);
    const mtlCount = files.filter((file) => getFileExtension(file) === "mtl").length;
    const textureCount = files.filter((file) => ["png", "jpg", "jpeg", "webp", "bmp", "gif"].includes(getFileExtension(file))).length;
    return {
      scene,
      mainFile: objFile,
  ...
  if (glbFile) {
    ...
    const textureCount = files.filter((file) => ["png", "jpg", "jpeg", "webp", "bmp", "gif"].includes(getFileExtension(file))).length;
    const binCount = files.filter((file) => getFileExtension(file) === "bin").length;
    return {
      scene,
      mainFile: glbFile,
  ...
  throw new Error("Select a GLB/glTF file, an OBJ with optional MTL and texture files, or a ZIP containing these files.");
}
...
function normaliseTextureColourCorrection(
  value: Partial<TextureColourCorrection> | Record<string, unknown> | null | undefined,
  fallback: TextureColourCorrection = defaultTextureColourCorrection,
): TextureColourCorrection {
  const src = value ?? {};
  const num = (key: keyof TextureColourCorrection, min: number, max: number): number => {
    const raw = (src as Record<string, unknown>)[key];
    const value = typeof raw === "number" ? raw : Number(raw);
    const fallbackValue = fallback[key];
    return Number.isFinite(value) ? Math.max(min, Math.min(max, value)) : fallbackValue;
  };
  return {
...
function sameTextureColourCorrection(a: TextureColourCorrection, b: TextureColourCorrection): boolean {
  return (
...
function TextureCorrectionSlider({
  ...
  return (
    <label className={`slider-row texture-correction-row${gradient ? " with-gradient" : ""}`}>
      ...
      <span className="slider-control">
        <input
          ...
          title={tip}
        />
        {gradient && <span className={`slider-track-guide ${gradient}`} aria-hidden="true" />}
      </span>
      <input
        className="number"
        type="number"
...
function ColorBlockMap({ report }: { report: TextureBakeReport | null }) {
  if (!report)
    return <p className="muted">No baked colours available yet.</p>;
  ...
  return (
    <div className="color-block-section">
      <div className="color-block-meta">
        ...
        <span>
          Coverage: {formatPercent(report.colorBlockCoveragePercent)}
        </span>
      </div>
      <div className="color-block-map" aria-label="Colour distribution as block map">
      ...
      <p className="muted note">
        The block map groups similar RGB colours. Block size represents frequency; ordering roughly follows the colour spectrum, with neutral greys at the end.
      </p>
    </div>
  );
}

type TextureTab = "load" | "correction" | "bake" | "export" | "checks" | "diagnostics" | "settings";
type TexturePreviewView = "right" | "front" | "left" | "back" | "top" | "bottom";
...
function dataUrlToFile(dataUrl: string, name: string, type = "application/octet-stream"): File {
...
function downloadTextFile(name: string, text: string, type = "application/json"): void {
...
export default function App({ onBakedObjHandoff, onStatusChange, shellTheme = "dark", reloadDataNonce = 0 }: TextureBakeAppProps = {}) {
  ...
  const [previewBackground, setPreviewBackground] = useState<TexturePreviewBackground>("auto");
  ...
  const [surfaceDetailMode, setSurfaceDetailMode] = useState<SurfaceDetailMode>(
    "baseColorOnly",
  );
  const [bakeColorMode, setBakeColorMode] = useState<BakeColorMode>("baseColor");
  const [textureSizeMode, setTextureSizeMode] = useState<TextureSizeMode>("original");
  ...
  const loadFiles = useCallback(async (filesInput: FileList | File[]): Promise<MeshDiagnostics | null> => {
    ...
    const steps = [
      t("progressReadFiles"),
      ...(hasZipInput ? [t("progressSearchZip"), t("progressExtractZip")] : []),
    ...
    try {
      ...
      setProgress({ title: t("progressLoadTitle"), steps, activeIndex: parseIndex });
      await waitForPaint(40);
      const parsed = await parseModelFiles(files);
      setProgress({ title: t("progressLoadTitle"), steps, activeIndex: parseIndex + 2 });
      await waitForPaint(20);
      const report = analyzeScene(parsed.scene, parsed.mainFile.name);
      setProgress({ title: t("progressLoadTitle"), steps, activeIndex: parseIndex + 3 });
  ...
  function applyTextureSettingsObject(settings: Record<string, unknown>): void {
    if (settings.bakeColorMode === "baseColor" || settings.bakeColorMode === "baseColorEmissive" || settings.bakeColorMode === "visibleExperimental") setBakeColorMode(settings.bakeColorMode);
    if (settings.textureSizeMode === "original" || settings.textureSizeMode === "8192" || settings.textureSizeMode === "4096" || settings.textureSizeMode === "2048" || settings.textureSizeMode === "1024" || settings.textureSizeMode === "512" || settings.textureSizeMode === "custom") setTextureSizeMode(settings.textureSizeMode);
    if (typeof settings.customTextureMaxSize === "number") setCustomTextureMaxSize(settings.customTextureMaxSize);
    if (settings.appliedBakeColourCorrection && typeof settings.appliedBakeColourCorrection === "object") {
      const next = normaliseTextureColourCorrection(settings.appliedBakeColourCorrection as Record<string, unknown>);
      setAppliedBakeColourCorrection(next);
      setPendingBakeColourCorrection(next);
    }
    if (settings.pendingBakeColourCorrection && typeof settings.pendingBakeColourCorrection === "object") {
      setPendingBakeColourCorrection(
        normaliseTextureColourCorrection(settings.pendingBakeColourCorrection as Record<string, unknown>, appliedBakeColourCorrection),
      );
    }
    if (settings.surfaceDetailMode === "baseColorOnly" || settings.surfaceDetailMode === "baseColorAndSurfaceMaps") setSurfaceDetailMode(settings.surfaceDetailMode);
    if (settings.reliefMode === "off" || settings.reliefMode === "heightBumpOnly" || settings.reliefMode === "aoRoughnessProxy") setReliefMode(settings.reliefMode);
    if (typeof settings.reliefStrengthPercent === "number") setReliefStrengthPercent(settings.reliefStrengthPercent);
    if (settings.reliefSmoothing === "off" || settings.reliefSmoothing === "light" || settings.reliefSmoothing === "strong") setReliefSmoothing(settings.reliefSmoothing);
    if (settings.subdivisionMode === "adaptive" || settings.subdivisionMode === "off") setSubdivisionMode(settings.subdivisionMode);
    if (settings.subdivisionQuality === "fast" || settings.subdivisionQuality === "medium" || settings.subdivisionQuality === "fine" || settings.subdivisionQuality === "veryFine" || settings.subdivisionQuality === "ultra" || settings.subdivisionQuality === "extreme" || settings.subdivisionQuality === "custom") setSubdivisionQuality(settings.subdivisionQuality);
    if (typeof settings.triangleBudget === "number") setTriangleBudget(settings.triangleBudget);
    if (typeof settings.maxSubdivisionDepth === "number") setMaxSubdivisionDepth(settings.maxSubdivisionDepth);
    if (typeof settings.exportScale === "number") setExportScale(settings.exportScale);
    if (settings.previewView === "right" || settings.previewView === "front" || settings.previewView === "left" || settings.previewView === "back" || settings.previewView === "top" || settings.previewView === "bottom") setPreviewView(settings.previewView);
    if (settings.previewBackground === "auto" || settings.previewBackground === "light" || settings.previewBackground === "dark") setPreviewBackground(settings.previewBackground);
    if (typeof settings.wireframe === "boolean") setWireframe(settings.wireframe);
    if (typeof settings.showAxes === "boolean") setShowAxes(settings.showAxes);
    if (typeof settings.syncPreviews === "boolean") setSyncPreviews(settings.syncPreviews);
  }

  async function handleSaveTextureProject(): Promise<void> {
    const steps = ["Collect settings", "Embed source files", "Write project JSON"];
    setProgress({ title: "Save Texture Baking project", steps, activeIndex: 0 });
    ...
    if (saveTextureProjectParts.sourceFiles && lastFilesRef.current?.length) {
      setProgress({ title: "Save Texture Baking project", steps, activeIndex: 1 });
      payload.sourceFiles = await Promise.all(lastFilesRef.current.map(async (file) => ({
        name: file.name,
        type: file.type || "application/octet-stream",
        dataUrl: await fileToDataUrl(file),
      })));
    }
    setProgress({ title: "Save Texture Baking project", steps, activeIndex: 2 });
    await waitForPaint(20);
  ...
  async function handleLoadTextureProjectFile(file: File): Promise<void> {
    ...
    try {
      ...
      if (loadTextureProjectParts.sourceFiles && Array.isArray(root.sourceFiles)) {
        setProgress({ title: "Load Texture Baking project", steps, activeIndex: 1 });
  ...
  const resolvedTextureMaxSize = useMemo(() => {
    ...
    if (textureSizeMode === "custom") return Math.max(128, Math.round(customTextureMaxSize));
  ...
  const runTextureBake = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Texture baking could not be completed.";
      setBakedScene(null);
      setBakeReport(null);
  ...
  const meshIssueCount = diagnostics
    ? diagnostics.openEdges + diagnostics.nonManifoldEdges + diagnostics.degenerateTriangles + diagnostics.trianglesWithoutUV
    : 0;
  const meshWarningCount = diagnostics
    ? diagnostics.thinTriangles + diagnostics.duplicateFaces + diagnostics.trianglesWithUVOutside01 + diagnostics.skinnedMeshCount
    : 0;
  ...
  function handleTextureSettingsResizePointerDown(event: React.PointerEvent<HTMLDivElement>) {
  ...
  return (
    <div className={`app ${darkMode ? "theme-dark" : "theme-light"}`}>
      ...
      <main
        className="layout"
        style={{ "--texture-settings-width": `${textureSettingsWidth}px` } as React.CSSProperties}
      >
        <section className="panel left-panel tabbed-panel texture-tabbed-panel" data-active-tab={activeTextureTab}>
          <nav className="sidebar-tabs texture-tabs" aria-label="Texture Baking workflow sections">
            {([
              ["load", "Load", "Load a GLB, ZIP, or OBJ package with its MTL and texture files."],
              ["correction", "Colour correction", "Adjust texture colours before baking."],
              ["bake", "Bake", "Bake selected texture channels into vertex or face colours."],
              ["export", "Export / Handoff", "Export the baked OBJ or send it directly to VertexColor 2 ColorMix."],
          ...
          <div className="card texture-section texture-load-section">
            <h2>Load model</h2>
            <p className="workflow-intro">Load a GLB, ZIP, or OBJ package with its MTL and texture files. Files are selected through the file dialog; drag and drop is disabled to keep the workflow consistent.</p>
            <label className={`file-drop staged-file-picker texture-file-picker ${busy ? "busy" : ""}`}>
              <span>GLB, ZIP, or OBJ+MTL+textures</span>
              <strong>{fileInfo?.name ?? (busy ? t("loadingModel") : "Choose file")}</strong>
              <input
                ref={inputRef}
                type="file"
            ...
            {fileInfo && (
              <div className="file-summary full">
                ...
                <div>
                  {formatBytes(fileInfo.size)} · {fileInfo.sourceSummary} · {formatNumber(fileInfo.fileCount)} file(s) ·{" "}
                  {diagnostics
          ...
          <div className="card texture-section texture-correction-section">
            ...
            <p className="workflow-intro">Adjust texture colours before baking. These values are applied to the original preview, the next bake, exported OBJ files and handoff to VertexColor 2 ColorMix.</p>

            <div className="active-correction-box texture-correction-summary">
              <b>{t("activeBakeColourCorrection")}</b>
              <div>
                {t("brightness")}: <b>{appliedBakeColourCorrection.brightness}</b> · {t("contrast")}: <b>{appliedBakeColourCorrection.contrast}</b> · {t("saturation")}: <b>{appliedBakeColourCorrection.saturation}</b> · {t("temperature")}: <b>{appliedBakeColourCorrection.temperature}</b> · {t("hue")}: <b>{appliedBakeColourCorrection.hue}</b> · {t("tint")}: <b>{appliedBakeColourCorrection.tint}</b> · {t("gamma")}: <b>{appliedBakeColourCorrection.gamma}</b>
              </div>
              <small>{t("bakeColourCorrectionApplied")}</small>
            </div>
            <h3 className="subhead">{t("prepareBakeColourCorrection")}</h3>
            <TextureCorrectionSlider label={t("brightness")} tip={t("tipBakeBrightness")} value={pendingBakeColourCorrection.brightness} min={-100} max={100} onChange={(value) => setPendingBakeColourCorrection((prev) => ({ ...prev, brightness: value }))} />
            <TextureCorrectionSlider label={t("contrast")} tip={t("tipBakeContrast")} value={pendingBakeColourCorrection.contrast} min={-100} max={100} onChange={(value) => setPendingBakeColourCorrection((prev) => ({ ...prev, contrast: value }))} />
            <TextureCorrectionSlider label={t("saturation")} tip={t("tipBakeSaturation")} value={pendingBakeColourCorrection.saturation} min={-100} max={100} onChange={(value) => setPendingBakeColourCorrection((prev) => ({ ...prev, saturation: value }))} />
            <TextureCorrectionSlider label={t("temperature")} tip={t("tipBakeTemperature")} value={pendingBakeColourCorrection.temperature} min={-100} max={100} onChange={(value) => setPendingBakeColourCorrection((prev) => ({ ...prev, temperature: value }))} />
            <TextureCorrectionSlider label={t("hue")} tip={t("tipBakeHue")} value={pendingBakeColourCorrection.hue} min={-180} max={180} gradient="hue" onChange={(value) => setPendingBakeColourCorrection((prev) => ({ ...prev, hue: value }))} />
            <TextureCorrectionSlider label={t("tint")} tip={t("tipBakeTint")} value={pendingBakeColourCorrection.tint} min={-100} max={100} gradient="tint" onChange={(value) => setPendingBakeColourCorrection((prev) => ({ ...prev, tint: value }))} />
            <TextureCorrectionSlider label={t("gamma")} tip={t("tipBakeGamma")} value={pendingBakeColourCorrection.gamma} min={0.2} max={3} step={0.05} onChange={(value) => setPendingBakeColourCorrection((prev) => ({ ...prev, gamma: Math.round(value * 100) / 100 }))} />
            <div className="adjustment-actions texture-correction-actions">
              <button
                type="button"
                onClick={() => {
                  const next = normaliseTextureColourCorrection(pendingBakeColourCorrection);
                  setPendingBakeColourCorrection(next);
                  setAppliedBakeColourCorrection(next);
                  setBakedScene(null);
                  setBakeReport(null);
              ...
              <button
                ...
                className="secondary"
                onClick={() => {
                  setPendingBakeColourCorrection(defaultTextureColourCorrection);
                  setAppliedBakeColourCorrection(defaultTextureColourCorrection);
                  setBakedScene(null);
                  setBakeReport(null);
                  setBakeError(null);
          ...
          <div className="card texture-section texture-bake-section">
            ...
            <p className="workflow-intro">Bake the selected texture channels into vertex or face colours. These settings affect the baked OBJ and the object handed off to VertexColor 2 ColorMix.</p>

            <h3 className="subhead">{t("basis")}</h3>
            <div className="inline-row">
              <InfoLabel tip={t("bakeColorSourcesTip")}>{t("bakeColorSources")}</InfoLabel>
              <select
                value={bakeColorMode}
                onChange={(event) =>
              ...
              >
                ...
                <option value="baseColorEmissive">Base Color + Emissive</option>
                <option value="visibleExperimental">{t("visibleExperimentalOption")}</option>
              </select>
            </div>
            <div className="inline-row">
              <InfoLabel tip={t("textureResolutionTip")}>{t("textureResolution")}</InfoLabel>
              <select
                value={textureSizeMode}
                onChange={(event) =>
                  setTextureSizeMode(event.currentTarget.value as TextureSizeMode)
                }
              >
            ...
            {textureSizeMode === "custom" && (
              <div className="inline-row">
                <InfoLabel tip={t("textureResolutionTip")}>{t("customTextureEdge")}</InfoLabel>
                <input
                  type="number"
                  min={128}
                  max={16384}
            ...
            {diagnostics && (
              <div className="inline-row subtle-row">
                ...
                <b>{diagnostics.baseColorTextureSizeSummary || diagnostics.textureSourceSizeSummary || "–"}</b>
              </div>
            )}
            ...
            <div className="inline-row">
              ...
              >
                <option value="adaptive">{t("subdivisionAdaptiveOption")}</option>
            ...
            <div className="inline-row">
              ...
              <select
                ...
                onChange={(event) => {
                  const next = event.currentTarget.value as SubdivisionQuality;
            ...
            {subdivisionQuality === "custom" && (
              <>
                <div className="inline-row">
                  <InfoLabel tip={t("triangleBudgetTip")}>{t("triangleBudget")}</InfoLabel>
                  <select
                ...
                <div className="inline-row">
                  <InfoLabel tip={t("maxDepthTip")}>{t("maxDepth")}</InfoLabel>
                  <input
                    ...
                    disabled={subdivisionMode === "off"}
                    onChange={(event) => {
                      setMaxSubdivisionDepth(Number(event.currentTarget.value));
                      setSubdivisionQuality("custom");
                    }}
                  />
                </div>
            ...
            <details className="advanced-options">
              ...
              <div className="inline-row">
                <InfoLabel tip={t("surfaceDetailSourcesTip")}>{t("surfaceDetailSources")}</InfoLabel>
                <select
                  ...
                  disabled={subdivisionMode === "off"}
                >
                  <option value="baseColorOnly">{t("surfaceBaseOnlyOption")}</option>
              ...
              <div className="inline-row">
                <InfoLabel tip={t("reliefGeometryTip")}>{t("reliefGeometry")}</InfoLabel>
                <select
                  value={reliefMode}
                  onChange={(event) =>
                ...
                >
                  ...
                  <option value="heightBumpOnly">{t("reliefHeightBumpOnlyOption")}</option>
                  <option value="aoRoughnessProxy">{t("reliefAoRoughnessProxyOption")}</option>
                </select>
              </div>
              <div className="inline-row">
                <InfoLabel tip={t("reliefStrengthTip")}>{t("reliefStrength")}</InfoLabel>
                <select
                  value={reliefStrengthPercent}
                  onChange={(event) =>
                    setReliefStrengthPercent(Number(event.currentTarget.value))
                  }
              ...
              <div className="inline-row">
                <InfoLabel tip={t("reliefSmoothingTip")}>{t("reliefSmoothing")}</InfoLabel>
                <select
                ...
                >
                  <option value="off">{t("reliefOffOption")}</option>
                  <option value="light">{t("reliefSmoothingLightOption")}</option>
                  <option value="strong">{t("reliefSmoothingStrongOption")}</option>
                </select>
              </div>
              <p className="muted note compact-note">
                Normal, AO, roughness and relief options are not active by default. They are experimental geometry or subdivision indicators only.
              </p>
            </details>
          ...
          <div className="card texture-section texture-export-section">
            ...
            <p className="workflow-intro">Export the baked OBJ as a separate file or send it directly to VertexColor 2 ColorMix. Check the export scale here before continuing.</p>
            {diagnostics?.boundingBox && (
              <div className="file-summary full texture-scale-summary">
                <div><b>Scale preview</b></div>
                {fileInfo?.name && <div>Source: {fileInfo.name}</div>}
                <div>Raw size: {diagnostics.boundingBox.size.map(formatDimension).join(" × ")}</div>
                <div>Export size: {diagnostics.boundingBox.size.map((value) => formatScaledDimension(value, exportScale)).join(" × ")} mm</div>
              </div>
            )}
            <div className="inline-row">
              <InfoLabel tip={t("exportScaleTip")}>{t("exportScale")}</InfoLabel>
              <select
                value={exportScale}
            ...
            <div className="texture-action-grid">
              ...
              <button
                ...
                disabled={!bakedScene || bakeBusy || exportBusy || !onBakedObjHandoff}
            ...
            <p className="muted note">
              Handoff uses the same baked OBJ without saving an intermediate file.
            </p>
          </div>

          <div className="card texture-section texture-checks-section">
            <h2>Checks</h2>
            <p className="workflow-intro">Review the baked result and colour distribution before exporting or handing off the model.</p>
            {!bakeReport ? (
              <p className="muted">No model baked yet. Load a model and run Texture Baking to show the checks.</p>
            ) : (
              <>
                <div className="stats">
                  <StatRow label="Source meshes" value={bakeReport.meshCount} />
                  <StatRow label="Original triangles" value={bakeReport.triangleCount} />
                  <StatRow
                  ...
                  <StatRow
                    label="Subdivision"
                    value={bakeReport.subdivisionMode === "off" ? "Off" : `${formatFloat(bakeReport.subdivisionFactor, 2)}×`}
                  ...
                  <StatRow
                    ...
                    value={formatBytes(bakeReport.estimatedBakedGeometryBytes)}
                    warning={bakeReport.estimatedBakedGeometryBytes > 800 * 1024 * 1024}
                  />
                  {bakeReport.reliefEnabled && (
                    <>
                      <StatRow label="Relief source" value={reliefSourceLabel(bakeReport.reliefSource)} />
                      <StatRow
                      ...
                      <StatRow
                        ...
                        value={formatDimension(bakeReport.reliefMaxDisplacement)}
                      />
                    </>
                  )}
                </div>
          ...
          {bakeReport && (
            <details className="card collapsible-card texture-section texture-checks-section">
              <summary>
                <InfoLabel tip={t("colorDistributionTip")}>{t("colorDistribution")}</InfoLabel>
              </summary>
          ...
          <div className="card diagnostics-card texture-section texture-diagnostics-section">
            <h2>Diagnostics</h2>
            <p className="workflow-intro">Use diagnostics for mesh, topology, UV and material issues. Structural problems should usually be repaired before baking.</p>
            {!diagnostics ? (
              <p className="muted">{t("notLoaded")}</p>
            ) : (
              <>
                <div className="diagnostic-summary">
                  <span className={meshIssueCount + bakeIssueCount > 0 ? "badge bad" : meshWarningCount > 0 ? "badge warn" : "badge ok"}>
                    {meshIssueCount + bakeIssueCount > 0 ? t("issuesDetected") : meshWarningCount > 0 ? t("warningsDetected") : t("noWarnings")}
                  </span>
                  <span className="muted">
                    Meshes {formatNumber(diagnostics.meshCount)} · triangles {formatNumber(diagnostics.triangleCount)} · open edges {formatNumber(diagnostics.openEdges)} · non-manifold {formatNumber(diagnostics.nonManifoldEdges)}
                  </span>
                </div>

                {(diagnostics.warnings.length > 0 || bakeReport?.warnings.length) && (
                  <ul className="warning-list compact">
                ...
                <details className="diagnostic-details" open={showDiagnosticsOpen}>
                  <summary>{showDiagnosticsOpen ? t("hideDetails") : t("showDetails")}</summary>

                  <h3>{t("meshDiagnostics")}</h3>
                  <div className="stats">
                    <StatRow label="Meshes" value={diagnostics.meshCount} />
                    <StatRow label="Skinned Meshes" value={diagnostics.skinnedMeshCount} warning={diagnostics.skinnedMeshCount > 0} />
                    <StatRow label="Vertices" value={diagnostics.vertexCount} />
                    <StatRow label="Unique positions" value={diagnostics.uniquePositionCount} />
                    <StatRow label="Duplicate positions / UV seams" value={diagnostics.duplicatePositionCount} />
                    <StatRow label="Triangles" value={diagnostics.triangleCount} warning={diagnostics.triangleCount > 800_000} />
                    <StatRow label="Estimated memory" value={formatBytes(memoryEstimate)} warning={memoryEstimate > 800 * 1024 * 1024} />
                  </div>

                  <h3>{t("topologyCheck")}</h3>
                  <div className="stats">
                    <StatRow label="Total edges" value={diagnostics.totalEdges} />
                    <StatRow label="Open edges" value={diagnostics.openEdges} danger={diagnostics.openEdges > 0} />
                    <StatRow label="Non-manifold edges" value={diagnostics.nonManifoldEdges} danger={diagnostics.nonManifoldEdges > 0} />
                    <StatRow label="Degenerate triangles" value={diagnostics.degenerateTriangles} danger={diagnostics.degenerateTriangles > 0} />
                    <StatRow label="Very thin triangles" value={diagnostics.thinTriangles} warning={diagnostics.thinTriangles > 0} />
                    <StatRow label="Duplicate faces" value={diagnostics.duplicateFaces} warning={diagnostics.duplicateFaces > 0} />
                  </div>

                  <h3>{t("uvMaterialCheck")}</h3>
                  <div className="stats">
                    <StatRow label="Materials" value={diagnostics.materialCount} />
                    <StatRow label="Base-colour textures" value={diagnostics.textureCount} warning={diagnostics.textureCount === 0} />
                    <StatRow label="Base-colour resolution" value={diagnostics.baseColorTextureSizeSummary || "–"} warning={diagnostics.baseColorTextureMaxSize > 4096} />
                    <StatRow label="Other texture resolutions" value={diagnostics.textureSourceSizeSummary || "–"} warning={diagnostics.textureSourceMaxSize > 4096} />
                    <StatRow label="Additional maps" value={`Emissive ${formatNumber(diagnostics.emissiveMapCount)} · Alpha ${formatNumber(diagnostics.alphaMapCount)} · Normal ${formatNumber(diagnostics.normalMapCount)} · AO/Roughness ${formatNumber(diagnostics.aoMapCount + diagnostics.roughnessMapCount)}`} warning={diagnostics.emissiveMapCount + diagnostics.alphaMapCount + diagnostics.normalMapCount + diagnostics.aoMapCount + diagnostics.roughnessMapCount > 0} />
                    <StatRow label="Transparency / special material" value={`transparent ${formatNumber(diagnostics.transparentMaterialCount)} · Transmission ${formatNumber(diagnostics.transmissionMaterialCount)} · Clearcoat ${formatNumber(diagnostics.clearcoatMaterialCount)}`} warning={diagnostics.transparentMaterialCount + diagnostics.transmissionMaterialCount + diagnostics.clearcoatMaterialCount > 0} />
                    <StatRow label="Meshes with UV" value={diagnostics.meshesWithUV} />
                    <StatRow label="Meshes without UV" value={diagnostics.meshesWithoutUV} danger={diagnostics.meshesWithoutUV > 0} />
                    <StatRow label="Triangles without UV" value={diagnostics.trianglesWithoutUV} danger={diagnostics.trianglesWithoutUV > 0} />
                    <StatRow label="UV outside 0–1" value={diagnostics.trianglesWithUVOutside01} warning={diagnostics.trianglesWithUVOutside01 > 0} />
                  </div>
                </details>
              </>
            )}
          ...
          {activeTextureTab === "settings" && (
            <div className="card texture-section texture-settings-section">
              <h2>Project data</h2>
              <p className="workflow-intro">Save or load the Texture Baking project data. The project file can contain the baking settings and the selected source model package.</p>
                ...
                <div className="project-settings-group">
                  ...
                  <label className="checkbox-row compact">
                    <input
                      ...
                      onChange={(event) => setSaveTextureProjectParts((prev) => ({ ...prev, settings: event.currentTarget.checked }))}
                    />
                  ...
                  <label className="checkbox-row compact">
                    <input
                      ...
                      onChange={(event) => setSaveTextureProjectParts((prev) => ({ ...prev, sourceFiles: event.currentTarget.checked }))}
                    />
                ...
                <div className="project-settings-group">
                  ...
                  <label className="checkbox-row compact">
                    <input
                      ...
                      onChange={(event) => setLoadTextureProjectParts((prev) => ({ ...prev, settings: event.currentTarget.checked }))}
                    />
                    <span>Baking settings</span>
                  </label>
                  <label className="checkbox-row compact">
                    <input
                      ...
                      onChange={(event) => setLoadTextureProjectParts((prev) => ({ ...prev, sourceFiles: event.currentTarget.checked }))}
                    />
                    <span>Source model/files</span>
                  </label>
                </div>
              ...
              <p className="muted note">Source files are embedded directly in the JSON project file when selected. Large GLB/ZIP/OBJ packages can therefore make the project file large.</p>

              <div className="settings-actions">
                <button type="button" className="secondary" onClick={() => void handleSaveTextureProject()} disabled={busy || bakeBusy || exportBusy} title={t("saveTextureProjectTip")}>
                  Save project
                </button>
                <button type="button" className="primary-action" onClick={() => textureProjectInputRef.current?.click()} disabled={busy || bakeBusy || exportBusy} title={t("loadTextureProjectTip")}>
                  Load project
                </button>
                <input
                ...
                <div className="preview-control-row">
                  <label>
                    ...
                    <select
                      ...
                      onChange={(event) => {
                        setPreviewView(event.currentTarget.value as TexturePreviewView);
                  ...
                  <label title="Sets the preview background independently from the UI theme.">
                    <InfoLabel tip="Sets the preview background independently from the UI theme.">Background</InfoLabel>
                    <select
                      value={previewBackground}
                      onChange={(event) => setPreviewBackground(event.currentTarget.value as TexturePreviewBackground)}
                    >
                  ...
                  <label>
                    <input
                      ...
                      checked={showAxes}
                      disabled={!hasAnyPreviewModel}
                      onChange={(event) => setShowAxes(event.currentTarget.checked)}
                    />
                  ...
                  <label>
                    <input
                      ...
                      disabled={!hasAnyPreviewModel}
                      onChange={(event) => setSyncPreviews(event.currentTarget.checked)}
                    />
                    <InfoLabel tip={t("syncViewsTip")}>Sync</InfoLabel>
                  </label>
                  <button className="secondary compact" type="button" disabled={!hasAnyPreviewModel} onClick={() => setPreviewFitSignal((value) => value + 1)} title={t("fitToModelTip")}>
                    {t("fitToModel")}
                  </button>
                  <button className="secondary compact" type="button" disabled={!hasAnyPreviewModel} onClick={() => setPreviewResetSignal((value) => value + 1)} title={t("resetViewTip")}>
                    {t("resetView")}
                  </button>
                  <button className="secondary compact" type="button" disabled={!hasAnyPreviewModel} onClick={() => setPreviewRebuildKey((value) => value + 1)} title={t("rebuildPreviewTip")}>
                    {t("rebuildPreview")}
                  </button>
                </div>
              </div>
            ...
            <div className="preview-content-scroll texture-preview-content">
              <div className="preview-grid">
                <div className="preview-cell">
                  <div className="preview-cell-title">{t("originalPreview")}</div>
                  <ModelPreview
                ...
                <div className="preview-cell">
                  ...
                  <ModelPreview
                    ...
                    emptyText={scene ? t("noBakedModel") : t("runTextureBaking")}
                    syncId="baked"
                    syncEnabled={syncPreviews}
                    syncState={cameraSyncState}
                    onSyncChange={handleCameraSyncChange}
